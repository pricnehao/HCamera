...
import com.haofugang.hfgcamera.internal.utils.DateTimeUtils;

import java.io.File;
import java.util.concurrent.TimeUnit;

import static com.haofugang.hfgcamera.internal.utils.Constant.MaxRECTime;
...
public class CameraControlPanel extends RelativeLayout
    ...
    //停止录制需要的时间,全局可见的录制时间
    private long ALLrecordingTimeSeconds = 0;
...
//        hasFlash = false;
    ...
    public void lockControls(int cameraType) {
        /**
         * 判断闪光灯显示和隐藏
         */
        if(cameraType == 0)
    ...
    public void allowCameraSwitching(boolean isAllowed) {
        cameraSwitchView.setVisibility(isAllowed ? VISIBLE : INVISIBLE);
    }
    ...
    private class TimerTask extends TimerTaskBase implements Runnable {
        ...
        public void start() {
            ...
            recordingTimeSeconds = 0;
            ALLrecordingTimeSeconds = recordingTimeSeconds;
            timerView.setText(String.format("%02d:%02d", recordingTimeMinutes, recordingTimeSeconds));
            timerView.setVisibility(View.VISIBLE);
            handler.postDelayed(this, DateTimeUtils.SECOND);
        }
